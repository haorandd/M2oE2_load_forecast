...
# ──────────────────────────────────────────────────────────────────────────────
# QRNN Model (drop-in replacement for WeekForecastProbLSTM)
# Keeps same API: forward(enc_l, enc_ext) -> (mu, logvar) with shapes [B, 168]
# Internally predicts quantiles (0.1, 0.5, 0.9) per horizon step.
# ──────────────────────────────────────────────────────────────────────────────
class WeekForecastProbQRNN(nn.Module):
    """
    Drop-in replacement:
    - Uses a Quantile Regression Neural Network (QRNN) style MLP.
    - Predicts quantiles {0.1, 0.5, 0.9} for each horizon step (168).
    - Converts quantiles to (mu, logvar) so the rest of the code (Gaussian NLL, CRPS,
      gaussian_icdf-based quantiles, Winkler) works without modification.

    NOTE: This preserves your script behavior without changing training/eval loops.
    """

    def __init__(self,
                 input_features: int,      # = 1 + K_ext (kept for compatibility)
                 hidden_size:    int = 128,
                 num_layers:     int = 2,
        ...
        self.num_q = len(self.q_levels)

        # MLP with LazyLinear so we don't need to know encoder length at init.
        layers = []
        ...
        # output: forecast_len * num_q quantiles
        layers.append(nn.Linear(hidden_size, forecast_len * self.num_q))
        self.net = nn.Sequential(*layers)

        # constant for mapping (q0.9 - q0.1) to sigma under Normal
        # z0.9 - z0.1 ≈ 2.5631031310892007
        self.register_buffer("_z_diff_09_01", torch.tensor(2.5631031310892007))
    ...
    def forward(self, enc_l, enc_ext=None):
        ...
        out = self.net(x)  # [B, forecast_len*num_q]
        q = out.view(-1, self.forecast_len, self.num_q)  # [B, H, Q]

        # enforce non-crossing quantiles by sorting along quantile dimension
        ...
        # Convert (q10, q50, q90) -> (mu, sigma) assuming an underlying Normal
        mu = q50
        sigma = (q90 - q10) / self._z_diff_09_01

        # Numerical safety: avoid zero/negative sigma
...
def train_prob_lstm(model, train_loader, *,
                    epochs: int, lr: float,
                    device: torch.device,
                    save_path: str = "_QRNN_best_model.pt"):
...
def evaluate_lstm_model(model,
                        test_loader,
                        device,
                        model_path="_QRNN_best_model.pt",
                        visualize=True,
                        n_vis_samples=5,
    ...
    """
    Same visual/metric style as RNN/ARIMA, but for WeekForecastProbLSTM.
    """
...
if __name__ == "__main__":
    ...
    model_name = f"QRNN_{output_len}hours"
    ...
    model_eval = WeekForecastProbQRNN(
        input_features = 1 + n_externals,
        hidden_size    = 128,
        num_layers     = 2,
        forecast_len   = 168,
    ...
    evaluate_lstm_model(model_eval,
                        ...
                        visualize=True,
                        n_vis_samples=5,
                        global_fig="mean_traj_var_QRNN.png")
    time2 = time.time()
