...
# ──────────────────────────────────────────────────────────────────────────────
# Q-ResNet Model Components (Based on "Improving Probabilistic Load Forecasting Using Quantile Regression NN With Skip Connections")
# ──────────────────────────────────────────────────────────────────────────────
class DualResidualBlock(nn.Module):
    """
    Standard building block for Q-ResNet utilizing skip connections.
    Mitigates gradient vanishing/exploding to allow deeper architectures.
    """
...
class WeekForecastProbQResNet(nn.Module):
    """
    Drop-in replacement:
    - Deep architecture with skip connections (Q-ResNet).
    - Predicts quantiles {0.1, 0.5, 0.9} per horizon step.
    - Converts quantiles to (mu, logvar) so Gaussian NLL/CRPS code works unchanged.
    """

    def __init__(self,
                 input_features: int,  # Kept for compatibility mapping
                 hidden_size: int = 128,
                 num_blocks: int = 3,  # Deeper network using skip connections
        ...
        # Stacked Dual Residual Blocks for Q-ResNet
        self.blocks = nn.ModuleList([
            DualResidualBlock(hidden_size, dropout) for _ in range(num_blocks)
        ])

        # Output: forecast_len * num_q quantiles
        self.out_layer = nn.Linear(hidden_size, forecast_len * self.num_q)

        # Constant for mapping (q0.9 - q0.1) to sigma under Normal distribution
    ...
    def forward(self, enc_l, enc_ext=None):
        ...
        out = self.out_layer(h)  # [B, forecast_len*num_q]
...
if __name__ == "__main__":
    ...
    model_name = f"QResNet_{output_len}hours"
    ...
    # Replaced WeekForecastProbResNetPlus with WeekForecastProbQResNet
    model = WeekForecastProbQResNet(
    ...
    model_eval = WeekForecastProbQResNet(
        input_features=1 + n_externals,
        hidden_size=128,
        num_blocks=3,
        forecast_len=168,
    ...
    evaluate_lstm_model(model_eval,
                        ...
                        visualize=True,
                        n_vis_samples=5,
                        global_fig="mean_traj_var_QResNet.png")
    time2 = time.time()
