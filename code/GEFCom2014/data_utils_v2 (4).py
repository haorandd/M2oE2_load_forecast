...
def _oncor_load_weekly_utils():
    ...
    This version can consume both the original historical file
    `TransformerLoadData.csv` and the newer forecast-style file
    `XFMR_forecast_data_20251113.csv` where time is split into
    DATE1 + HOUR1 and temperature is given as TEMPERATURE.
    '''
    csv_path_old = "TransformerLoadData.csv"
    csv_path_new = "XFMR_forecast_data_20251113.csv"
    out_dir      = "processed_data"
    # os.makedirs(out_dir, exist_ok=True)
    os.makedirs(out_dir, exist_ok=True)

    # --- Load & harmonize raw CSVs ---
    dfs = []

    # 1) Historical file (already in final schema with DATEHRLWT)
    if os.path.exists(csv_path_old):
        df_old = pd.read_csv(csv_path_old)
        df_old.columns = [c.strip() for c in df_old.columns]
        dfs.append(df_old)

    # 2) New forecast-style file (DATE1 + HOUR1, TEMPERATURE)
    if os.path.exists(csv_path_new):
        df_new = pd.read_csv(csv_path_new)
        df_new.columns = [c.strip() for c in df_new.columns]

        # Build DATEHRLWT if needed from DATE1 + HOUR1
        if "DATEHRLWT" not in df_new.columns:
            if "DATE1" in df_new.columns and "HOUR1" in df_new.columns:
                base_dt = pd.to_datetime(df_new["DATE1"], errors="coerce")
                # HOUR1 may be int/float; treat as hours from midnight
                hour_delta = pd.to_timedelta(df_new["HOUR1"].astype(float), unit="h")
                df_new["DATEHRLWT"] = base_dt + hour_delta
            else:
                raise ValueError(
                    "XFMR_forecast_data_20251113.csv is missing DATE1/HOUR1 "
                    "and there is no DATEHRLWT column to construct timestamps."
                )

        # Align temperature column name with historical schema
        if (
            "SURFACETEMPERATUREFAHRENHEIT" not in df_new.columns
            and "TEMPERATURE" in df_new.columns
        ):
            df_new = df_new.rename(
                columns={"TEMPERATURE": "SURFACETEMPERATUREFAHRENHEIT"}
            )
    ...
    if not dfs:
        raise FileNotFoundError(
            "No ONCOR source CSVs found. Expected at least one of "
            "'TransformerLoadData.csv' or 'XFMR_forecast_data_20251113.csv'."
        )
    ...
    # Some rows (e.g. aggregated statistics) may not have XFMR; default to 'ALL'
    ...
    # 关键一步：把这些列里的 NaN 填成 0
    # 尤其是新数据没有的一些气象列，否则每周都会因为 NaN 被整周丢掉
    if present_features:
        df[present_features] = df[present_features].fillna(0.0)

    def build_weekly_tensor(gdf, features, week_hours=168):
        # Keep only relevant columns
        gdf = (
            gdf[[time_col] + features]
            .copy()
        ...
        for w in range(num_weeks):
            ...
            # Only keep fully observed weeks (reindex 时可能插入 NaN)
        ...
        if week_arrays:
            ...
            time_index = np.stack(week_times, axis=0)                 # (W,168)
    ...
    for xfmr, g in df.groupby(group_col):
        ...
        print(
            f"XFMR={xfmr}: weeks={tensor.shape[0]} (out of {possible_weeks}), "
            f"hours={total_hours}, saved to {out_file}"
        )
...
def get_season_from_month(month: int) -> int:
    return {12: 0, 1: 0, 2: 0, 3: 1, 4: 1, 5: 1, 6: 2, 7: 2, 8: 2, 9: 3, 10: 3, 11: 3}[month]


def get_data_smartds_weekly(
    csv_path,
    profile_ids=None,
    smooth_sigma=1.0,
):
    """
    Return per-profile weekly blocks for SMART-DS hourly data.
    ...
    df = pd.read_csv(csv_path)
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df = df.sort_values(["profile_id", "timestamp"]).reset_index(drop=True)

    if profile_ids is not None:
        df = df[df["profile_id"].isin(profile_ids)].copy()
    ...
    for pid, g in df.groupby("profile_id"):
        ...
        # 这里假设你已经提前保证 hourly 完整
        # 每周 168 小时，共 52 周 = 8736 小时，最后 24 小时先丢掉
        n_full_weeks = len(g) // 168
        if n_full_weeks < 2:
            continue

        g = g.iloc[: n_full_weeks * 168].copy()

        times_all = g["timestamp"].to_numpy()
        load_all = g["load_kw"].astype(float).to_numpy()
        temp_all = g["temperature"].astype(float).to_numpy()

        workday_all = (g["timestamp"].dt.dayofweek >= 5).astype(int).to_numpy()
        season_all = g["timestamp"].dt.month.map(get_season_from_month).astype(int).to_numpy()
        ...
        for w in range(n_full_weeks):
            ...
            t_week = times_all[s:e]
            l_week = load_all[s:e]
            tmp_week = temp_all[s:e]
            wd_week = workday_all[s:e]
            ss_week = season_all[s:e]

            if smooth_sigma is not None and smooth_sigma > 0:
                l_week = gaussian_filter1d(l_week, sigma=smooth_sigma)
                tmp_week = gaussian_filter1d(tmp_week, sigma=smooth_sigma)

            times_w.append(t_week)
            load_w.append(l_week)
            temp_w.append(tmp_week)
            workday_w.append(wd_week)
            season_w.append(ss_week)

        blocks.append(
            {
                "profile_id": pid,
                "load_type": g["load_type"].iloc[0],
                "times": np.array(times_w, dtype=object),
                "load": np.array(load_w, dtype=float),
                "temp": np.array(temp_w, dtype=float),
                "workday": np.array(workday_w, dtype=int),
                "season": np.array(season_w, dtype=int),
            }
        )
...
def process_seq2seq_data_profile_aware(
    blocks,
    *,
    train_ratio=0.7,
    output_len=24,
    ...
    """
    blocks: list of dicts returned by get_data_smartds_weekly()

    New behavior:
    - load is scaled PER PROFILE
    - temp/workday/season/temp_fc_* are scaled GLOBALLY
    - sample construction is done WITHIN each profile only
    """
    import numpy as np
    import pandas as pd
    import torch
    ...
    # ---------- global scalers for context features ----------
    base_global_features = ["temp", "workday", "season"]
    scalers = {}

    for feat in base_global_features:
        all_vals = np.concatenate([np.asarray(b[feat], dtype=float).flatten() for b in blocks])
        sc = MinMaxScaler()
        sc.fit(all_vals.reshape(-1, 1))
        scalers[feat] = sc

    # ---------- per-profile scaler for load ----------
    scalers["load"] = {}
    for b in blocks:
        pid = b["profile_id"]
        vals = np.asarray(b["load"], dtype=float).flatten()
        sc = MinMaxScaler()
        sc.fit(vals.reshape(-1, 1))
        scalers["load"][pid] = sc

    # ---------- build oracle 24h temp summary features ----------
    def compute_forward_24h_temp_stats(temp_1d, horizon=24):
    ...
    extra_names = ["temp_fc_mean24", "temp_fc_max24", "temp_fc_min24", "temp_fc_ramp24"]
    extra_vals = {k: [] for k in extra_names}

    for b in blocks:
        temp_flat = np.asarray(b["temp"], dtype=float).flatten()
        mean24, max24, min24, ramp24 = compute_forward_24h_temp_stats(temp_flat, horizon=24)

        b["temp_fc_mean24"] = mean24.reshape(b["temp"].shape)
        b["temp_fc_max24"]  = max24.reshape(b["temp"].shape)
        b["temp_fc_min24"]  = min24.reshape(b["temp"].shape)
        b["temp_fc_ramp24"] = ramp24.reshape(b["temp"].shape)

        extra_vals["temp_fc_mean24"].append(mean24)
        extra_vals["temp_fc_max24"].append(max24)
        extra_vals["temp_fc_min24"].append(min24)
        extra_vals["temp_fc_ramp24"].append(ramp24)

    # ---------- global scalers for temp_fc_* ----------
    for feat in extra_names:
        sc = MinMaxScaler()
        sc.fit(np.concatenate(extra_vals[feat]).reshape(-1, 1))
        scalers[feat] = sc

    ext_keys = ["temp", "workday", "season",
                "temp_fc_mean24", "temp_fc_max24", "temp_fc_min24", "temp_fc_ramp24"]
    ...
    # ---------- build samples WITHIN each profile ----------
    for b in blocks:
        pid = b["profile_id"]
        n_weeks = b["load"].shape[0]
        need_weeks = encoder_len_weeks + decoder_len_weeks
        if n_weeks < need_weeks:
            continue
        ...
        # load -> profile-specific scaler
        vec_load = np.asarray(b["load"], dtype=float).flatten()
        proc["load"] = scalers["load"][pid].transform(vec_load.reshape(-1, 1)).flatten()

        # ext features -> global scalers
        for feat in ext_keys:
            vec = np.asarray(b[feat], dtype=float).flatten()
            proc[feat] = scalers[feat].transform(vec.reshape(-1, 1)).flatten()
        ...
        for w in range(last_start + 1):
            ...
            enc_l = proc["load"][enc_start:enc_end]
            dec_full_l = proc["load"][dec_start:dec_end]

            enc_ext = np.stack([proc[k][enc_start:enc_end] for k in ext_keys], axis=-1)
            dec_ext = np.stack([proc[k][dec_start:dec_start + L] for k in ext_keys], axis=-1)
...
def process_seq2seq_data_profile_aware_using_scalers(
    blocks,
    *,
    scalers,
    train_ratio=0.7,
    ...
    """
    Same idea as process_seq2seq_data_using_scalers(), but:
    - load uses a PROFILE-SPECIFIC scaler
    - temp/workday/season/temp_fc_* use GLOBAL scalers
    - samples are built WITHIN each profile only
    """
    import numpy as np
    import pandas as pd
    import torch
    ...
    for b in blocks:
        ...
        # build temp_fc_* for this profile
        temp_flat = np.asarray(b["temp"], dtype=float).flatten()
        mean24, max24, min24, ramp24 = compute_forward_24h_temp_stats(temp_flat, horizon=24)

        b_local = dict(b)
        b_local["temp_fc_mean24"] = mean24.reshape(b["temp"].shape)
        b_local["temp_fc_max24"]  = max24.reshape(b["temp"].shape)
        b_local["temp_fc_min24"]  = min24.reshape(b["temp"].shape)
        b_local["temp_fc_ramp24"] = ramp24.reshape(b["temp"].shape)

        processed = {}

        # ---------- load -> profile-specific scaler ----------
        vec_load = np.asarray(b_local["load"], dtype=float).flatten()
        if "load" not in scalers or pid not in scalers["load"]:
            raise KeyError(f"Missing load scaler for profile_id='{pid}'")
        processed["load"] = scalers["load"][pid].transform(vec_load.reshape(-1, 1)).flatten()

        # ---------- external features -> global scalers ----------
        for k in ext_keys:
            vec = np.asarray(b_local[k], dtype=float).flatten()
            if k not in scalers:
                raise KeyError(f"Missing scaler for key='{k}'. Keys={list(scalers.keys())}")
            processed[k] = scalers[k].transform(vec.reshape(-1, 1)).flatten()
