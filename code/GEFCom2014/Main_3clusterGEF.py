...
from Model_3cluster_GEF import VariationalSeq2Seq_meta
...
def process_seq2seq_data_gefcom_direct_hourly(
    ...
    *,
    ...
    # ---------------------------------------------------------------
    # Temperature-station clustering.
    # Fit station groups using TRAINING years only, then keep the same
    # station-to-cluster mapping for train / validation / test.
    # k_temp=1 recovers the original 25-station average-temperature setup.
    # ---------------------------------------------------------------
    k_temp = int(k_temp)
    if not (1 <= k_temp <= len(temp_cols)):
        raise ValueError(f"k_temp must be in [1, {len(temp_cols)}], got {k_temp}.")

    train_temp_mask = full.index.year.isin(list(train_years))
    train_temp = full.loc[train_temp_mask, temp_cols].to_numpy(dtype=float)
    if train_temp.shape[0] == 0:
        raise ValueError(f"No temperature rows found for train_years={train_years}.")
    if not np.isfinite(train_temp).all():
        raise ValueError("Non-finite training temperatures detected before station clustering.")

    if k_temp == 1:
        raw_labels = np.zeros(len(temp_cols), dtype=int)
    else:
        station_corr = np.corrcoef(train_temp, rowvar=False)
        station_corr = np.nan_to_num(station_corr, nan=0.0, posinf=1.0, neginf=-1.0)
        station_distance = np.clip(1.0 - station_corr, 0.0, 2.0)
        np.fill_diagonal(station_distance, 0.0)
        ...
        raw_labels = clusterer.fit_predict(station_distance)

    # Re-label clusters deterministically by the first station index.
    raw_groups = []
    for raw_id in sorted(np.unique(raw_labels)):
        members = [temp_cols[i] for i in np.where(raw_labels == raw_id)[0].tolist()]
        raw_groups.append(members)
    raw_groups.sort(key=lambda members: min(int(name[1:]) for name in members))
    ...
    temp_cluster_info = {
        "k_temp": k_temp,
        "fit_years": [int(y) for y in train_years],
        "method": "agglomerative_correlation_distance_average_linkage",
        "clusters": [],
    }
    temp_current_keys = []
    for cluster_id, members in enumerate(raw_groups):
        key = f"temp_c{cluster_id:02d}"
        data[key] = full[members].mean(axis=1).astype(float)
        temp_current_keys.append(key)
        temp_cluster_info["clusters"].append({
            "cluster_id": int(cluster_id),
            "feature_key": key,
            "stations": list(members),
        })

    print(f"[INFO] Temperature clustering: k_temp={k_temp}, fitted only on train_years={tuple(train_years)}")
    for info in temp_cluster_info["clusters"]:
        print(f"  - {info['feature_key']}: {info['stations']}")
    ...
    # Future temperature inputs. We assume these shifted values represent
    # the temperature forecasts available to the day-ahead predictor.
    temp_future_keys = []
    for key in temp_current_keys:
        for h in range(output_len):
            fc_key = f"{key}_fc_tplus{h:02d}"
            data[fc_key] = data[key].shift(-h)
            temp_future_keys.append(fc_key)
    data = data.ffill().bfill()
    
    # ------------------------------------------------------------------
    # Flexible expert/external-variable selection.
    # Available variables:
    #   temp    -> [cluster temperatures, cluster-wise future 24h forecast paths]
    ...
    if "temp" in ext_variables:
        keys = list(temp_current_keys) + list(temp_future_keys)
...
if __name__ == "__main__":
    ...
    print('latent_dim/hidden dim = 64, clustering, logvar_max = -3.8')
    ...
    # Number of anonymous temperature-station clusters.
    # k_temp=1 recovers the original 25-station average-temperature baseline.
    k_temp = 3
    ...
    model_name = "M2OE2_3cluster"
    model_path = "M2OE2_3cluster.pt"
    results_path = "M2OE2_3cluster_eval_results.json"
    print(f"[INFO] model_name: {model_name}")
    print(f"[INFO] model_path: {model_path}")
    print(f"[INFO] results_path: {results_path}")

    # Latest cleaned data + train-fitted temperature-station clusters.
    ...
    print(f"[INFO] k_temp: {k_temp}")
    print(f"[INFO] temperature clusters: {temp_cluster_info['clusters']}")
    ...
    eval_summary = {
        ...
        "ext_variables": list(ext_variables),
        "k_temp": int(k_temp),
        "temperature_clustering": temp_cluster_info,
