# -*- coding: utf-8 -*-
"""
CNN-GRU Probabilistic Forecasting Framework with Low-Rank Adaptation (LoRA)

This script implements a hybrid CNN-GRU architecture for probabilistic time-series forecasting.
The workflow demonstrates a transfer learning approach:
1.  **Pre-training**: The model is trained on a source domain (aggregated Oncor data).
...
from data_utils import * # for reconstruct_sequence
...
def gaussian_nll_loss(mu: torch.Tensor, logvar: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        target: Ground truth values [B, T].
        
    Returns:
        Scalar mean NLL value.
    """
    nll = 0.5 * (logvar + math.log(2 * math.pi) + (target - mu).pow(2) / logvar.exp())
    return nll.mean()
...
class WeekForecastProbCNNGRU(nn.Module):
    """
    CNN-GRU Hybrid Architecture for Probabilistic Forecasting.

    This model combines the feature extraction capabilities of Convolutional Neural Networks (CNN)
    with the temporal modeling of Gated Recurrent Units (GRU).
    
    The forward pass proceeds as follows:
    1.  Input Concatenation: Load data and external covariates are concatenated.
    2.  Feature Extraction: Two 1D convolution layers extract local temporal patterns.
    3.  Sequence Modeling: A GRU layer captures long-term dependencies.
    4.  Probabilistic Output: A linear head predicts the mean (mu) and log-variance (logvar).
    """
    def __init__(self,
                 input_features: int,
    ...
    def forward(self, enc_l, enc_ext):
        """
        ...
        """
        if enc_ext is None or enc_ext.numel() == 0:
            x_cat = enc_l
        else:
            x_cat = torch.cat([enc_l, enc_ext], dim=-1)
        x = x_cat.permute(0, 2, 1)
...
def evaluate_cnn_gru_model(model,
    ...
    for batch in test_loader:
        ...
        if visualize:
            ...
            # 1. Plot Sample with History Context
...
if __name__ == "__main__":
    ...
    # CNN-GRU Architecture parameters
    conv_channels = (32, 64)
    kernel_size   = 3
    gru_hidden    = 128
    num_layers    = 1
    ...
    model_arch   = f"CNNGRU_C{conv_channels[0]}-{conv_channels[1]}_K{kernel_size}_H{gru_hidden}_Enc{encoder_len_weeks}w"
    ...
    if not os.path.isfile(BASE_CKPT):
        ...
        model_base = WeekForecastProbCNNGRU(
    ...
    model_lora = WeekForecastProbCNNGRU(
        input_features=input_features,
        conv_channels=conv_channels,
        kernel_size=kernel_size,
        gru_hidden=gru_hidden,
    ...
    t1 = time.time()
    evaluate_cnn_gru_model(model_lora,
                       test_loader_target,
                       device,
                       model_path=LORA_CKPT,
                       visualize=True,
                       data_name=TARGET_TAG,
                       model_name=f"CNNGRU_LORA_r{lora_r}",
                       data_export_list=plot_data_frames
                       )
    print("LoRA Model Eval wall time (s):", time.time() - t1)
    ...
    model_base_eval = WeekForecastProbCNNGRU(
        input_features=input_features,
        conv_channels=conv_channels,
        kernel_size=kernel_size,
        gru_hidden=gru_hidden,
    ...
    t1 = time.time()
    evaluate_cnn_gru_model(model_base_eval,
                       test_loader_target,
                       device,
                       model_path=BASE_CKPT, 
                       visualize=True,
                       data_name=TARGET_TAG, 
                       model_name="CNNGRU_BASE_ZeroShot",
                       data_export_list=plot_data_frames
                       )
    print("Base Model (Zero-Shot) Eval wall time (s):", time.time() - t1)
    ...
    if plot_data_frames:
        ...
        xlsx_path_cnngru = "cnngru_comparison_data.xlsx"
        try:
            final_plot_df.to_excel(xlsx_path_cnngru, index=False)
            print(f"CNNGRU plot data saved to {xlsx_path_cnngru}")
        except ImportError:
